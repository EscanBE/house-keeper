	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(workingDir) > 0 {
			...
			if os.IsNotExist(err) {
				libutils.PrintfStdErr("specified working directory does not exists: %s\n", workingDir)
				os.Exit(1)
			}
			if !workingDirInfo.IsDir() {
				libutils.PrintfStdErr("specified working directory is not a directory: %s\n", workingDir)
				os.Exit(1)
			}
		}
		...
		if len(outputFileName) > 0 {
			...
			if err == nil && outputInfo.IsDir() {
				libutils.PrintfStdErr("specified output is a directory: %s\n", outputFileName)
				os.Exit(1)
			}
			actualFileName := filepath.Base(outputFileName)
			if actualFileName != outputFileName {
				libutils.PrintfStdErr("specified output file name is not just a file name, should be: %s. You can specify the directory to download into using flag --%s\n", actualFileName, flagWorkingDir)
				os.Exit(1)
			}
		}
...
func init() {
	downloadCmd.PersistentFlags().Int(
		flagConcurrentDownload,
		4,
		"number of concurrent downloads (only supported for aria2c)",
	)

	downloadCmd.PersistentFlags().String(
		flagWorkingDir,
		"",
		"working directory",
	)
